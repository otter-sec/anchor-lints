use anchor_lints_utils::{
    diag_items::{
        anchor_inner_account_type, is_anchor_account_loader_type, is_anchor_spl_token_account_type,
    },
    mir_analyzer::AnchorContextInfo,
    utils::has_account_constraint,
};
...
pub fn extract_init_accounts_and_inner_types<'tcx>(
    ...
    if let TyKind::Adt(adt_def, generics) = accounts_struct_ty.kind() {
        ...
        for field in &variant.fields {
            ...
            let is_account_loader = is_anchor_account_loader_type(cx.tcx, account_ty);
            if !has_account_constraint(cx, field, "init") {
                continue;
            }
            if let Some(inner_ty) = anchor_inner_account_type(cx.tcx, account_ty) {
                // Skip standard SPL token account types - they're initialized by Anchor automatically
                if is_anchor_spl_token_account_type(cx.tcx, inner_ty) {
                    continue;
                }
                res.insert(
                    account_name,
